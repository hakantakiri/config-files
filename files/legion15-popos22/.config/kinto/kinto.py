...
define_conditional_modmap(lambda wm_class: wm_class.casefold() not in terminals and wm_class.casefold() not in ["dota2".casefold()],{

    # Key.CAPSLOCK: Key.RIGHT_CTRL,   # Caps2Cmdq
...
# define_keymap(lambda wm_class: wm_class.casefold() not in remotes and wm_class.casefold() not in ["code".casefold()], {
    ...
    #Custom replacement for the one above
    K("Alt-Tab"): K("Alt-Tab"), 
    ...
    # K("Right"): [K("Shift-Right")],


    # Enabled
    K("RC-Right"): K("End"),                      # End of Line
    # K("RC-Shift-Right"): K("Shift-Tab"),          # Select all to End of Line # Modified
...
# # For kitty
# define_keymap(re.compile("kitty"), {
#     K("LC-Alt-j"): pass_through_key,
#     # K("Alt-j"):    K("Ctrl-F1"),
#     # K("Alt-j"):    K("Ctrl-F1"),
#     # K("LC-Up"):   K("Ctrl-t")
#     # K("LC-Tab-t"):   K("Ctrl-F1"),
#     K("LC-Up"):   escape_next_key,
# }, "kitty")
...
games = [game.casefold() for game in games]
gamesStr = "|".join(str('^'+x+'$') for x in games)

define_conditional_modmap(re.compile(gamesStr, re.IGNORECASE), {
    # Test, make meta key work as prin-modifier-key
...
define_keymap(re.compile(gamesStr), {
    K("RC-Up"): K("Super-Up"),
    # K("Alt-Tab"): K("Alt-Tab"), 
})
...
define_keymap(re.compile("kitty"), {
    K("LC-Up"): K("Super-Up"),
    K("LC-Alt-j"): K("Super-Alt-j"),
    K("LC-Alt-u"): K("Super-Alt-u"),
    K("LC-Alt-i"): K("Super-Alt-i"),
    K("LC-Alt-k"): K("Super-Alt-k"),
    K("LC-Alt-Key_7"): K("Super-Alt-Key_7"),
    K("LC-Alt-Key_8"): K("Super-Alt-Key_8"),
    K("LC-Alt-m"): K("Super-Alt-m"),
    K("LC-Alt-dot"): K("Super-Alt-dot"),
    K("LC-Alt-left"): K("Super-Alt-left"),
    K("LC-Alt-right"): K("Super-Alt-right"),
    K("LC-Alt-up"): K("Super-Alt-up"),
    K("LC-Alt-down"): K("Super-Alt-down"),
})

# define_keymap(re.compile("code"), {
#     K("RC-Shift-Left"): K("Home"),
#     K("RC-Shift-Right"): K("End"),
# })

# For vs code and vs codium, swap LC to LA
# define_conditional_modmap(re.compile(codeStr, re.IGNORECASE), {
...
# })
# define_keymap(re.compile(code Str, re.IGNORECASE), {
#     K("Alt-Up"): K("Super-Up"),
#     K("Super-Up"): K("Alt-UP")
# })


# This works, this swaps ctrl-alt for a (select everything)
# define_keymap(re.compile("Gedit"), {
#     K("Alt-a"): K("Ctrl-a"),
#     K("Ctrl-a"): K("Alt-a")
# })

# define_keymap(re.compile("steam|^steam$|Gedit|dota2"), {
# This worked for dota
#     K("Alt-W"): K("Ctrl-W"),
#     K("Ctrl-W"): K("Alt-W"),

#     # Key.LEFT_ALT: Key.LEFT_CTRL,   # WinMac
#     # Key.LEFT_CTRL: Key.LEFT_ALT,    # WinMac
#     # K("Ctrl"): K("Alt")
#     # K("Alt"): K("Super")
#     # K("Super"):  K("Alt"),
#     # K("Alt-j"):    K("Ctrl-F1"),
...
# restart kinto : sudo systemctl restart xkeysnail